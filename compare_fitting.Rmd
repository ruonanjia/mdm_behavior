...
fit2 <- read.csv('MDM_model_fitting_parameters.csv', header = TRUE)
...
Clean data sheet
```{r}
fit2 = fit2[fit2$model == 'ambigNrisk' & fit2$fitby == 'rating', ]

# included subjects
ids = unique(log$id[log$is_exclude_behavior == 0])
...
fit_new = fit1 %>% select((id),(is_med),(alpha),(beta),(gamma),(val1), (val2), (val3), (val4), (LL),(AIC),(BIC),(model))

fit_old = fit2 %>% select((id),(is_med),(alpha),(beta),(gamma),(LL),(AIC),(BIC),(model))
fit_old$val1 <- NaN
fit_old$val2 <- NaN
fit_old$val3 <- NaN
fit_old$val4 <- NaN
# fit_old = fit_old %>% rename(alpha2=alpha, beta2=beta, gamma2 = gamma)

fit_all <- merge(fit_new, fit_old, by = intersect(names(fit_new), names(fit_old)))

fit_all <- rbind(fit_new, fit_old)
...
plot
```{r}
fit_plot <- data_summary(fit_all, varname = "BIC", groupnames=c("is_med", "model"))
...
fit_plot <- data_summary(fit_all, varname = "AIC", groupnames=c("is_med", "model"))
